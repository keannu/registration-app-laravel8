	...
	<div class="min-w-screen min-h-screen bg-gray-900 flex items-center justify-center px-5 py-5">
		...
		<div class="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden" style="max-width:1000px">
			<div class="md:flex w-full">
				...
				<div class="w-full md:w-1/2 py- px-5 md:px-5s">
					<div class="text-center mt-8 mb-2">
						<h1 class="font-bold text-3xl text-gray-900">{{ sSelectedText }}</h1>
						<p>{{ sSelectedDescriptionText }}</p>
					</div>
					<div class="w-full rounded h-auto m-auto flex flex-col rounded-xl mb-5" x-data="{ bIsLoginSelected: true }">
						<div class="relative w-full mt-4 rounded-md border h-10 p-1 bg-gray-200">
							<div class="relative w-full h-full flex items-center">
								<div class="w-full flex justify-center text-gray-400 cursor-pointer" @click="toggleSelected">
									<button>Login</button>
								</div>
								<div class="w-full flex justify-center text-gray-400 cursor-pointer" @click="toggleSelected">
							...
							<span 
								:class="{ 'left-1/2 -ml-1': (bIsLoginSelected === false), 'left-1': (bIsLoginSelected === true) }"
								class="text-indigo-400 font-semibold bg-white shadow text-sm flex items-center justify-center w-1/2 rounded h-[1.88rem] transition-all duration-150 ease-linear top-[4px] absolute">
								{{ sSelectedText }}
							</span>
						</div>
					</div>
					<div v-if="(bIsLoginSelected === true)" class="mx-2">
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-5">
								<label for="" class="text-xs font-semibold px-1">Username / Email</label>
								<div class="flex">
									...
									<input v-model="oLoginInfo.username" type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Username / Email">
								</div>
							</div>
						</div>
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-12">
								...
								<div class="flex">
									...
									<input v-model="oLoginInfo.password" type="password" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Password">
								</div>
							</div>
						</div>
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-5">
								<button class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-bold" :class="{ 'opacity-50 cursor-not-allowed': bIsPageLoading }" @click="loginUser">
									<div v-if="(bIsPageLoading === true)" class="inline-block h-5 w-5 animate-spin rounded-full border-4 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]" role="status">
					...
					<div v-else class="mx-2">
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-2">
								...
								<div class="flex">
									<div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><em class="mdi mdi-account-outline text-gray-400 text-lg"></em></div>
									<input v-model="oRegisterInfo.username" type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Username">
								</div>
							</div>
						</div>
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-2">
								...
								<div class="flex">
									...
									<input v-model="oRegisterInfo.email" type="email" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Email Address">
								</div>
							</div>
						</div>
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-2">
								...
								<div class="flex">
									...
									<input v-model="oRegisterInfo.phone_no" type="text" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Phone Number" @input="limitInputToNumbers">
								</div>
							</div>
						</div>
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-2">
								...
								<div class="flex">
									<div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><em class="mdi mdi-lock-outline text-gray-400 text-lg"></em></div>
									<input v-model="oRegisterInfo.password" type="password" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Password">
								</div>
							</div>
						</div>
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-5">
								<label for="" class="text-xs font-semibold px-1">Confirm Password<span class="text-red-400"> *</span></label>
								<div class="flex">
									<div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><em class="mdi mdi-lock-outline text-gray-400 text-lg"></em></div>
									<input v-model="oRegisterInfo.confirm_password" type="password" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Confirm Password">
								</div>
							</div>
						</div>
						<div class="flex -mx-3">
							<div class="w-full px-3 mb-5">
								<button class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold" :class="{ 'opacity-50 cursor-not-allowed': bIsPageLoading }" @click="registerUser">
									<div v-if="(bIsPageLoading === true)" class="inline-block h-5 w-5 animate-spin rounded-full border-4 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]" role="status">
										<span class="!absolute !-m-px !h-px !w-px !overflow-hidden !whitespace-nowrap !border-0 !p-0 ![clip:rect(0,0,0,0)]">Loading...</span>
									</div>
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters('oUser', [ 'bIsSavingSuccessful' ]),
			 ...
			 */
			sSelectedText() {
				return (this.bIsLoginSelected === true) ? 'Login' : 'Register';
			},

			/**
			 * sSelectedDescriptionText
			 * @author Keannu Rim Kristoffer C. Regala <keannu>
			 * @since 2023.05.18
			 * @return string
			 */
			sSelectedDescriptionText() {
				return (this.bIsLoginSelected === true) ? 'Enter proper credential to login' : 'Enter proper data to register';
			}
		},
		methods: {
			...mapActions('oUser', [ 'login', 'storeUser' ]),
			...
			toggleSelected() {
				...
				this.bIsLoginSelected = !this.bIsLoginSelected;
			},
			...
			async loginUser() {
				...
				await this.login(this.oLoginInfo)
			...
			limitInputToNumbers(oEvent) {
				this.oRegisterInfo.phone_no = oEvent.target.value.replace(/\D/g, '');
			},
			...
			async registerUser() {
				...
				if (this.aErrors.length > 0) {
					...
					this.bIsPageLoading = true;
					await this.storeUser(this.oRegisterInfo);
					this.bIsPageLoading = false;
			...
			validateUserInfo() {
				...
				if (this.oEmailRegex.test(this.oRegisterInfo.email) === false) {
					this.aErrors.push('Invalid email format.');
				}

				if (this.oRegisterInfo.password !== this.oRegisterInfo.confirm_password) {
					this.aErrors.push('Password do not match.');
				}

				if (this.oRegisterInfo.phone_no.length > 0 && this.oRegisterInfo.phone_no.length < 11) {
			 ...
			 */
			checkRequireFields() {
				let aRequiredFields = [ 'username', 'email', 'password', 'confirm_password' ];
				for (const sKey of aRequiredFields) {
					if (this.oRegisterInfo[sKey].length <= 0) {
